...
#define VALVE_PIN       7
#define DUMMY_LED_PIN   3
#define RAIN_SENSE      A0
#define SOIL_MOIST      A1

//MQTT Topic
#define VALVE_ON      "farmer/node1/valve"
  ...
  if(newDataAvbl == true)
  {
        ...
        if (strcmp(gTopic, VALVE_ON) == 0)
        {
          if(inBuf[0] == '1')
          {
            ...
            digitalWrite(DUMMY_LED_PIN, HIGH);
          }
          else if(inBuf[0] == '0')
          {
            digitalWrite(VALVE_PIN, LOW);
            digitalWrite(DUMMY_LED_PIN, LOW);
          }
          else
          {
...
void getSensorData()
{
  ...
  float percentage = 0;
  //Read sensor data
  int chk = analogRead(RAIN_SENSE);
  percentage = (((1023.0 - (float)chk)/1023.0)*100.0);
//  Serial.println(percentage);
  
  buffer[0] = 0;
  dtostrf(percentage, 5, 2, buffer);
  client.publish("farmer/node1/rainsense",buffer);
  
  chk = analogRead(SOIL_MOIST);
  percentage = (((1023.0 - (float)chk)/1023.0)*100.0);
//  Serial.println(chk);
  
  buffer[0] = 0;
  dtostrf(percentage, 4, 2, buffer);
  client.publish("farmer/node1/soilmoist",buffer);        
}
